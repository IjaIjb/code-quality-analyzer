...
export class IssueFilterManager {
  ...
  static filterIssues(issues: CodeIssue[], filters: IssueFilters): CodeIssue[] {
    return issues.filter((issue) => {
      ...
      const matchesSeverity =
        filters.severity.size === 0 || filters.severity.has(issue.type);

      // Filter by category
      const matchesCategory =
        filters.categories.size === 0 || filters.categories.has(issue.category);
  ...
  static createDefaultFilters(): IssueFilters {
    return {
      severity: new Set(["error", "warning", "info"] as const) as Set<
        "error" | "warning" | "info"
      >,
